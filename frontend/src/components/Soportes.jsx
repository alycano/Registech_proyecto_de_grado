...
const Soportes = ({ usuario, esAdmin, esInventario }) => {
        ...
        axios.post(API_ROUTES.ACTUALIZAR_MANTENIMIENTOS, {
            ...
            .then((response) => {
                ...
                Swal.fire({
                    ...
                    text:
                        response.data?.mensaje ||
                        'El equipo está nuevamente disponible',
                    confirmButtonText: 'Ok'
                })
    ...
    // =========================================================
    // DAR DE BAJA: NO SE PUEDE REPARAR
    // =========================================================

    const darDeBajaEquipo = (orden) => {
        Swal.fire({
            icon: 'warning',
            title: '¿El equipo no se puede reparar?',
            html: `
                <div class="text-start">
                    <p><strong>Equipo:</strong> ${orden.equipo || 'Equipo'}</p>
                    <p><strong>Número de serie:</strong> ${orden.num_serie}</p>
                    <p><strong>Diagnóstico:</strong> ${orden.falla}</p>
                    <p class="mb-2">
                        El equipo será marcado como <strong>Baja</strong>
                        y no podrá utilizarse hasta que un administrador
                        lo reintegre.
                    </p>
                    <p class="mb-0">
                        Indica el motivo:
            ...
            `,
            input: 'textarea',
            inputPlaceholder: 'Ejemplo: Daño irreparable en la tarjeta madre...',
            inputAttributes: {
                maxlength: 500
            },
            showCancelButton: true,
            ...
            preConfirm: (motivo) => {
                if (!motivo || !motivo.trim()) {
                    Swal.showValidationMessage(
                        'Debes indicar por qué no se puede reparar el equipo'
                    )
                    return false
                }
            ...
            axios.post(API_ROUTES.DAR_DE_BAJA_REPORTE, {
                num_serie: orden.num_serie,
                id_historial: orden.id_historial,
                tecnico: usuario,
                solucion: result.value
            })
                .then(response => {
                    Swal.fire({
                        icon: 'success',
                                    ...
                                    {ordenDetalle.fecha_solucion && (
                                        ...
                                        <div className="mt-3">
                                            ...
                                            <small className="text-secondary">
                                                ...
                                                Técnico:
                                                {' '}
                                                {ordenDetalle.nombre_tecnico ||
                                                    ordenDetalle.usuario_tecnico ||
                                                    '—'}
